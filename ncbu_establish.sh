...
docker run -ti \
	...
	-v /var/run/docker.sock:/var/run/docker.sock:ro \	#volume - link the docker.sock to enable docker commands from within the ncbu container.
	-v nextcloud-app:/mnt/nextcloud-app:ro \		#volume - define the nextcloud app volume for mounting within the ncbu container.
	-v nextcloud-db:/mnt/nextcloud-db:ro \			#volume - define the nextcloud database volume for mounting within the ncbu container.
