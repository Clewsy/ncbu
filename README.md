...
* Sensitive details/credentials can be entered directly into the *docker-compose.yml* file, or (as per this example) reference an external *.env* file (e.g. the password for the nextcloud MariaDB database is defined in *.env* by a line: `MARIADB_NEXTCLOUD_MYSQL_PASSWORD=secure_password`)
